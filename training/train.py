...
from models import *
from utils.datasets_2d_ivf import get_2d_ivf_loader
from utils.datasets_3d_ivf import get_3d_ivf_loader
import requests  # Added for TelegramReporter
...
class TelegramReporter:
    def __init__(self, token, chat_id, exp_name=""):
        self.token = token
        ...
        if self.enabled:
            _logger.info(f"Telegram reporting enabled for chat {self.chat_id}")

    def report_epoch(self, epoch, train_metrics, val_metrics, is_best=False, cm_path=None):
        if not self.enabled:
            return
            
        msg = f"🚀 **[{self.exp_name}] Epoch {epoch}**\n"
        if is_best:
            msg += "\n🌟🌟🌟 **NEW BEST MODEL!**\n"
        
        # Train Metrics
        msg += "📉 **Train:**\n"
        msg += f"  • Loss: {train_metrics.get('loss', 0):.4f}\n"

        
        # Val Metrics
        msg += "📊 **Val:**\n"
        msg += f"  • Loss: {val_metrics.get('loss', 0):.4f}\n"
        msg += f"  • Acc@1: {val_metrics.get('top1', 0):.3f}%\n"
        msg += f"  • Acc@3: {val_metrics.get('top3', 0):.3f}%\n"
        

        sent_msg_with_photo = False
        if cm_path and os.path.exists(cm_path):
            photo_api_url = f"https://api.telegram.org/bot{self.token}/sendPhoto"
            try:
                with open(cm_path, 'rb') as photo:
                    requests.post(photo_api_url, data={
                        "chat_id": self.chat_id,
                        "caption": msg,
                        "parse_mode": "Markdown"
                    }, files={"photo": photo}, timeout=15)
                sent_msg_with_photo = True
            except Exception as e:
                _logger.warning(f"Failed to send Telegram photo (CM): {e}")

        if not sent_msg_with_photo:
            try:
                ...
                _logger.warning(f"Failed to send Telegram message: {e}")

    def send_start_message(self, args):
        if not self.enabled:
            return
            
        msg = f"🚀 **STARTED TRAINING: [{self.exp_name}]**\n\n"
        msg += "⚙️ **Configuration:**\n"
        msg += f"  • Model: `{args.model}`\n"
        msg += f"  • Epochs: `{args.epochs}`\n"
        msg += f"  • Batch Size: `{args.batch_size}`\n"
        msg += f"  • Learning Rate: `{args.lr}`\n"
        msg += f"  • Optimizer: `{args.opt}`\n"
        msg += f"  • Scheduler: `{args.sched}`\n"
        if args.use_2d_csv:
            msg += f"  • Mode: `2D EmbryoMambaNet (Phase 1)`\n"
    ...
    def send_data_stats(self, class_stats: dict):
        """Send class distribution stats to Telegram."""
        if not self.enabled:
            return
        msg = f"📊 **[{self.exp_name}] Dataset Distribution**\n\n"
        for split, counts in class_stats.items():
            if len(counts) > 0 and isinstance(list(counts.values())[0], dict):
                total = sum(d.get('total', 0) for d in counts.values())
                msg += f"**{split.upper()}** ({total} clips):\n"
                for cls, info in sorted(counts.items(), key=lambda x: x[1].get('total', 0), reverse=True):
                    n = info.get('total', 0)
                    real = info.get('real', 0)
                    pct = 100.0 * n / total if total > 0 else 0
                    msg += f"  • {cls}: {n} ({pct:.1f}%) [Real: {real}]\n"
            else:
        ...
        try:
            ...
            _logger.warning(f"Failed to send Telegram data stats: {e}")
...
group.add_argument('--use-2d-csv', action='store_true', default=False,
                    help='Use 2D image dataloader (balance_dataset_v5_2d output) for embryo classification')
group.add_argument('--use-3d', action='store_true', default=False,
                    help='Use 3D clip dataloader (balance_dataset_v5_3d output) for embryo classification')
group.add_argument('--num-frames', type=int, default=5,
                    help='Number of frames per clip for 3D model (default: 5)')
...
group.add_argument('--encoder-checkpoint', default='', type=str, metavar='PATH',
                    help='Encoder checkpoint for frozen encoder models (default: none)')
...
group.add_argument('--lr', type=float, default=1e-4, metavar='LR',
                    help='learning rate (default: 1e-4)')  # FIX: 1e-3 too high for 7-class small dataset
...
group.add_argument('--early-stopping-patience', type=int, default=10, metavar='N',
                    help='early stopping patience in epochs, 0 to disable (default: 10)')
group.add_argument('--lambda-ordinal', type=float, default=0.5,
                    help='Weight for ordinal loss in EmbryoCompositionLoss (default: 0.5)')
group.add_argument('--lambda-count', type=float, default=0.3,
                    help='Weight for blastomere count MSE loss (default: 0.1)')  # FIX: Lower after count targets fix (FIX-1)
group.add_argument('--hba-mode', type=str, default='full',
                    choices=['full', 'eca', 'simam'],
                    help='Local attention mode in MobileNetV4IVFBlock: '
                         'full=SimAM+CA+SA+ECA, eca=ECA only, simam=zero-param (default: full)')
group.add_argument('--telegram-token', type=str, default='',
                    help='Telegram Bot API Token for metrics reporting')
group.add_argument('--telegram-group', type=str, default='',
                    help='Telegram Chat ID for metrics reporting')
...
def main():
    ...
    if getattr(args, 'phase2', False):
        _logger.info('Phase 2 mode: auto-enabled --use-2d-csv')
        args.use_2d_csv = True
    args.device = 'cuda:0'
    args.world_size = 1
    ...
    model = create_model(
        ...
        encoder_checkpoint=args.encoder_checkpoint,  # For frozen encoder models
    ...
    if args.dataset == 'alzheimer_3branch':
        from utils.alzheimer_dataset_3d import get_alzheimer_dataloaders
        
        # Default to 256x256 (Original) if input_size not specified
        if args.input_size is not None:
             h, w = args.input_size[1], args.input_size[2]
        else:
             h, w = 256, 256
             
        loader_train, loader_eval = get_alzheimer_dataloaders(
        ...
        # Hack to bypass creating loader again
        dataset_train = loader_train.dataset
        dataset_eval = loader_eval.dataset
        collate_fn = None
        args.prefetcher = False # Disable prefetcher for custom loader to ensure input is moved to cuda in training loop
    ...
    # 2D IVF dataloader (EmbryoMambaNet Phase 1)
    if args.use_2d_csv:
        _logger.info('Using 2D IVF dataloader (EmbryoMambaNet)')
        args.prefetcher = False  # custom loader returns (img, label) directly
        loader_train, dist_train = get_2d_ivf_loader(args, is_training=True)
        loader_eval,  dist_val   = get_2d_ivf_loader(args, is_training=False)
        class_stats = {'train': dist_train, 'val': dist_val}

    # 3D IVF dataloader (Mamba2VisionMorph Phase 1)
    elif getattr(args, 'use_3d', False):
        _logger.info(f'Using 3D IVF dataloader (Mamba2VisionMorph, T={getattr(args, "num_frames", 5)})')
        args.prefetcher = False
        loader_train, dist_train = get_3d_ivf_loader(args, is_training=True,  split='train')
        loader_eval,  dist_val   = get_3d_ivf_loader(args, is_training=False, split='val')
        class_stats = {'train': dist_train, 'val': dist_val}

    elif args.dataset != 'alzheimer_3branch':
    ...
    if args.dataset != 'alzheimer_3branch' and not args.use_2d_csv and not getattr(args, 'use_3d', False):
    ...
    # setup loss function
    if "embryoswin_plusplus" in args.model:
        from models.ivf_2d_swin_loss import EmbryoSwinLoss
        train_loss_fn = EmbryoSwinLoss(num_classes=args.num_classes, smoothing=args.smoothing)
        validate_loss_fn = train_loss_fn
        _logger.info(f"Using EmbryoSwinLoss (SupCon + LabelSmoothing) for model {args.model}")
    elif args.use_2d_csv or getattr(args, 'use_3d', False):
        # Use LabelSmoothingCrossEntropy for embryo classification (no count loss)
        if args.smoothing:
        ...
        _logger.info(
            f"Using LabelSmoothingCrossEntropy: smoothing={args.smoothing}"
        )
        validate_loss_fn = train_loss_fn
    elif args.jsd_loss:
        ...
        if args.dataset == 'alzheimer_3branch':
             # Weighted Loss for Imbalance (CN: 29%, MCI: 52%, AD: 17%)
             # Weights ~ [1.8, 1.0, 3.1] for [CN, MCI, AD]
             weights = torch.tensor([1.8, 1.0, 3.1]).cuda()
             train_loss_fn = nn.CrossEntropyLoss(weight=weights)
             print(f"Using Weighted CrossEntropyLoss: {weights}")
        else:
             train_loss_fn = nn.CrossEntropyLoss()
    train_loss_fn = train_loss_fn.cuda()
    if not args.use_2d_csv and not getattr(args, 'use_3d', False):
    ...
    if args.rank == 0:
        ...
        saver = timm.utils.CheckpointSaver(model, optimizer, args=args, amp_scaler=loss_scaler,
                                            model_ema=model_ema, checkpoint_dir=output_dir, max_history=args.checkpoint_hist)
        
        # Init Telegram Reporter
        telegram_reporter = TelegramReporter(
            token=args.telegram_token,
            chat_id=args.telegram_group,
            exp_name=exp_name
        )
        # Send start message
        if args.rank == 0:
            telegram_reporter.send_start_message(args)
            if args.use_2d_csv and 'class_stats' in dir():
                telegram_reporter.send_data_stats(class_stats)
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.rank == 0:
                ...
                if saver is not None:
                    ...
                    is_best = False
                    if best_metric is None or (eval_metric in eval_metrics and eval_metrics[eval_metric] > best_metric):
                        is_best = True
                    ...
                    cm_path = None
                    if args.num_classes == 7 and 'preds' in eval_metrics:
                        import matplotlib
                        matplotlib.use('Agg')  # non-interactive backend, avoid tkinter warning
                        import matplotlib.pyplot as plt
                        import seaborn as sns
                        from sklearn.metrics import confusion_matrix
                        try:
                            cm = confusion_matrix(eval_metrics['targets'], eval_metrics['preds'], labels=range(7))
                            plt.figure(figsize=(10, 8))
                            sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                                        xticklabels=['1-tPNf', '2-t2', '3-t3+', '5-t5+', '7-t7+', '9-t9+', '10-tM+'],
                                        yticklabels=['1-tPNf', '2-t2', '3-t3+', '5-t5+', '7-t7+', '9-t9+', '10-tM+'])
                            plt.xlabel('Predicted Stage')
                            plt.ylabel('True Stage')
                            plt.title(f'Confusion Matrix Epoch {epoch}')
                            cm_path = os.path.join(output_dir, f'cm_epoch_{epoch}.png')
                            plt.savefig(cm_path, bbox_inches='tight')
                            plt.close()
                        except Exception as e:
                            _logger.warning(f"Failed to generate confusion matrix: {e}")


                    # Report to Telegram
                    telegram_reporter.report_epoch(
                        epoch=epoch,
                    ...
                    _logger.info(f"*** Best {eval_metric} metric: {best_metric} @ epoch {best_epoch} ***")

                # Early stopping check
                if args.early_stopping_patience > 0 and best_epoch is not None:
                    epochs_since_improvement = epoch - best_epoch
                    if epochs_since_improvement >= args.early_stopping_patience:
                        if args.local_rank == 0:
                            _logger.info(
                                f'Early stopping triggered! No improvement for {args.early_stopping_patience} epochs. '
                                f'Best {eval_metric}: {best_metric:.4f} at epoch {best_epoch}'
                            )
                        break
...
def train_one_epoch(
    ...
    for batch_idx, batch_data in enumerate(loader):
        ...
        # Handle 5-frame dataloader (returns 3 items) vs standard (returns 2 items)
        if len(batch_data) == 3:
            input, target, metadata = batch_data
        else:
        ...
        with amp_autocast(dtype=args.dtype):
            if getattr(args, 'phase2', False) and metadata is not None:
    ...
    result = OrderedDict([('loss', losses_m.avg)])
    return result
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(loader):
            ...
            if not args.prefetcher:
                input = input.cuda()
                target = target.cuda()
                if args.use_2d_csv and metadata is not None:
                    metadata = metadata.cuda()
            if args.channels_last:
                input = input.contiguous(memory_format=torch.channels_last)
            ...
            if args.distributed:
                ...
                acc3 = utils.reduce_tensor(acc3, args.world_size)
            else:
    ...
    if all_preds:
        metrics['preds'] = torch.cat(all_preds).numpy()
        metrics['targets'] = torch.cat(all_targets).numpy()
