...
def select_frames_stage_balanced(
    ...
    Logic mới (v2):
      ...
      3. Stage < quota:
         - Với '1-tPNf' (stage đầu tiên, không có stage trước):
           Dùng augmentation (flip/rotate/photometric) để đạt quota
         - Với các stage từ '2-t2' trở lên:
           KHÔNG duplicate frame, thay vào đó lấy frame từ stage TRƯỚC (transition frames)
           để model học được các giai đoạn chuyển tiếp.
           Nếu vẫn thiếu sau khi lấy transition frames, mới dùng augmentation.

    Lý do:
      - Duplicate frame làm model học pattern lặp lại, không học được dynamics
      - Frame từ stage trước (transition) giúp model học boundary giữa các stage
      - Đây là thông tin quan trọng nhất cho IVF classification (phân biệt t3+ vs t5+)

    Returns:
        {stage: [(frame_path, aug_id)]}
        aug_id=-1: frame gốc | aug_id=0..N: augmented variant
    """
    ...
    # Xây dựng ordered list của stages có mặt trong embryo này
    ordered_stages = [s for s in active_stages if s in present]

    for stg_idx, stg in enumerate(ordered_stages):
        frames = present[stg]
        n = len(frames)
        ...
        if n < quota:
            ...
            if stg_idx == 0:
                # Stage đầu tiên (tPNf): không có stage trước → dùng augmentation
                ...
                while filled < shortfall:
                    ...
                    entries.append((src, aug_id_counter))
                    filled += 1
                    aug_id_counter += 1
            else:
                # Stage từ t2 trở lên: lấy frame từ stage TRƯỚC (transition frames)
                # Ưu tiên frame CUỐI của stage trước (gần boundary nhất)
                prev_stg = ordered_stages[stg_idx - 1]
                prev_frames = present[prev_stg]

                # Lấy transition_keep frame cuối của stage trước
                # Đây là các frame ngay trước khi chuyển sang stage hiện tại
                n_prev = len(prev_frames)
                transition_from_prev = prev_frames[max(0, n_prev - transition_keep):]

                # Thêm transition frames từ stage trước (aug_id=-1, frame gốc)
                filled = 0
                for fp in reversed(transition_from_prev):  # frame gần boundary nhất trước
                    if filled >= shortfall:
                        break
                    entries.append((fp, -1))
                    filled += 1

                # Nếu vẫn còn thiếu sau transition frames → dùng augmentation
                remaining = shortfall - filled
                if remaining > 0:
                    # Augment frame đầu của stage hiện tại (gần boundary nhất)
                    aug_pool = frames[:transition_keep] + frames[max(0, n - transition_keep):]
                    aug_pool = aug_pool * (remaining // max(len(aug_pool), 1) + 2)
                    aug_id_counter = 0
...
def run_pipeline(
    ...
    for eid in tqdm(embryo_info, desc="Write"):
        ...
        if sp == 'train':
            ...
            # Val/Test: giữ nguyên toàn bộ frame gốc, không cân bằng
        ...
        # QUAN TRỌNG: frame từ stage trước được thêm vào selected[stg_current]
        # → chúng được gán nhãn là stage hiện tại (transition learning)
        # Điều này đúng: model học nhận biết frame cuối stage trước = đầu stage sau
        frame_to_stage: Dict[Tuple[Path, int], str] = {}
        for stg, entries in selected.items():
            for fp, aug_id in entries:
                # Nếu frame này đã được map (từ stage trước), giữ nhãn stage hiện tại
                # (override) vì đây là transition frame được dùng cho stage này
    ...
    if not dry_run and copy_tasks:
        ...
        def _do_write(task):
            ...
            try:
                ...
                # Apply CLAHE+Sobel cho TẤT CẢ frame (gốc lẫn augmented)
                # → DataLoader không cần apply lại, nhẹ hơn khi training
                arr = apply_clahe_sobel(Image.fromarray(arr))
                arr.save(dst, 'JPEG', quality=95)
            except Exception as e:
                logging.error(f"Write error {src} aug{aug_id}: {e}")
